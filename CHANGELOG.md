...
## [Unreleased]
### Changed
- Closes #503 (rabbit-cage) — `hooks/session-start-dispatcher.py` now wires mechanical auto-resume of the rabbit-auto-evolve loop. After the welcome block it INVOKES `.claude/features/rabbit-auto-evolve/scripts/check-auto-resume.py` (a cross-scope INVOKE declared in `contract.md` `invokes.scripts`, NOT a cross-feature edit) and consumes its `{resume, action}` JSON verdict. On `resume: true` it surfaces a branded `Auto-resuming rabbit-auto-evolve loop — running /rabbit-auto-evolve start` banner in `systemMessage` and injects the action into `additionalContext` so Claude Code self-resumes after a restart. On `resume: false`, or when the script is absent / non-zero / times out / emits malformed JSON, SessionStart behaviour is unchanged (graceful degradation). New spec Inv 34; `rabbit-cage` bumped 5.37.0 -> 5.38.0. New e2e `test/test-session-start-auto-resume-wired.py` (resume-true surfaces banner+action; resume-false unchanged; running-marker suppresses; absent-script degrades gracefully).
