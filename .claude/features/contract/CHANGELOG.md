...
- **v2.5.0 (retire `surface`; `manifest` is the single artifact source of truth, #468):** `feature.json` historically carried two overlapping fields — `surface` (a lossy, drift-prone mirror of a feature's published artifacts) and `manifest` (the operational source that `rabbit-cage/install.py` walks to execute publish verbs). This release retires `surface` as a required/authoritative field at the SCHEMA + VALIDATION layer (contract scope only). `feature.json.schema.json` drops `surface` from `required` and documents it as deprecated with `manifest` as the source of truth; `lib/checks.py validate_feature` no longer requires `surface` (absence is valid) and only validates its shape when it is present (backwards-compatible deprecation so features carrying both still pass). `feature-json-template.json` drops the `surface` block in favor of `manifest: []` so new features inherit the manifest-only shape, and contract's own `feature.json` drops its `surface` block (dogfooding). Spec Inv 18 extended to mandate the surface-optional / manifest-authoritative invariant. New E2E regression `test/test-feature-surface-optional.py` asserts: schema does not require `surface`; `validate_feature` + the CLI shim pass on a manifest-only feature with no `surface`; both-present still validates; neither-present validates. Individual features' removal of `surface` from their own `feature.json` is a follow-up per-feature sweep (out of scope here — cross-feature writes).
