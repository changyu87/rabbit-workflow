#!/usr/bin/env python3
"""test-check-sentinel-scans-py.py — Inv 24.
...
# t1b: script must NOT walk .sh files (Python-only stack per Inv 11)
if '.sh' in src:
    print("FAIL t1b: script still references .sh (Python-only stack per Inv 11)", file=sys.stderr)
    FAIL = 1
else:
