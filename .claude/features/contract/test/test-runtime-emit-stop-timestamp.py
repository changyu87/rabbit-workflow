...
# (ix) with an INJECTED fixed aware `now` and the DEFAULT display zone (local,
# no `display-timezone` configurable declared in this temp repo), the rendered
# HH:MM:SS is the injected instant CONVERTED into the resolved display zone
# (machine local), not the bare UTC instant — per Inv 67 emit_stop_timestamp
# now renders via resolve_display_tz. Pin an aware `now` in a +05:00 zone and
# assert the rendered clock equals that same instant in the LOCAL zone.
with tempfile.TemporaryDirectory() as td:
    os.makedirs(os.path.join(td, ".claude", "features"))
    tz = datetime.timezone(datetime.timedelta(hours=5), name="TZ5")
    now = datetime.datetime(2026, 6, 4, 14, 32, 7, tzinfo=tz)  # UTC 09:32:07
    local_tz = datetime.datetime.now().astimezone().tzinfo
    expected = now.astimezone(local_tz).strftime("%H:%M:%S")
    utc_clock = now.astimezone(datetime.timezone.utc).strftime("%H:%M:%S")
    ...
    elif not text.startswith(expected + " "):
        fail(
            f"ix: injected-now must render LOCAL-converted {expected} "
            f"(not bare UTC {utc_clock}); got {text!r}"
        )
    else:
        ok(f"ix: injected aware now renders display-zone wall-clock ({text!r})")
...
with tempfile.TemporaryDirectory() as td:
    ...
    now = datetime.datetime(2026, 6, 4, 3, 13, 5, tzinfo=tz)  # on-boundary -> next fire 03:43
    ...
    # Both lines convert `now` into the resolved display zone (local, default).
    local_tz = datetime.datetime.now().astimezone().tzinfo
    ...
    if "next tick" not in idle_text or m_eta is None:
        ...
        if clk_min != now_min:
            fail(
                ...
                f"({now_local.strftime('%H:%M')}); got clock {clock_text!r}"
            )
        # ETA must be at/after now (same hour here: 03:13 fire >= 03:13 now).
            ...
            fail(
                ...
                f"eta={idle_text!r} now={now_local.strftime('%H:%M')} — UTC/local mismatch?"
            )
        else:
            ok(
