...
2. (removed in CONTRACT-BACKLOG-27) `dispatch-feature-edit.py` was deleted as dead production code (no runtime caller). The policy-block sentinel `RABBIT-POLICY-BLOCK-v1` is now consumed only by `policy-block.py` and the tdd-subagent dispatch path; that wiring is asserted by `test-policy-block.py`.
...
5. The `[rabbit]` print system is split into three artifacts (CONTRACT-BACKLOG-20):
   ...
   (c) `rabbit_print.py` — the shared renderer. Consumers (rabbit-cage
       ...
       the renderer.
6. (removed in CONTRACT-BACKLOG-27) `workspace-map.py` was deleted as orphan production code (its sole consumer was the retired `rabbit-workspace-map` skill).
7. `workspace-structure.json` exists at `.claude/features/contract/schemas/workspace-structure.json`, is valid JSON, and defines a node-tree schema: documents conforming to it must have `schema_version`, `owner`, `root`, `nodes` at top level; each node must have `name`, `required`, `description`, `children`. (The companion `workspace-map.json.schema.json` was deleted in CONTRACT-BACKLOG-27 along with its sole producer.)
8. (removed in CONTRACT-BACKLOG-27) `rabbit-workspace-map/SKILL.md` was deleted (source and deployed copy) as orphan surface. The build-contract entry for it is also removed.
...
27. (removed in CONTRACT-BACKLOG-27) `dispatch-feature-edit.py` was deleted; its path-detection invariant no longer applies.
...
31. (removed in CONTRACT-BACKLOG-27) `check-no-main-edits.py` was deleted; it was never auto-invoked, and the feature-touch branch + PR workflow plus the `Bash(git push * main)` deny rules in rabbit-cage Inv 51 are the live guards against direct edits to `main`/`master`.
