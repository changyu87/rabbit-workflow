...
Numbering preserves gaps at 6, 7, 30, 35 for retired invariants — each is tombstoned in `CHANGELOG.md`. Stable numbers protect cross-references in code, tests, and other features' specs from cascading rewrites; gaps are permitted by `check_invariant_monotonic_order` (strictly increasing, not contiguous).
...
29. Every producer that emits a `[rabbit]` message MUST go through `rabbit_print.py`. The canonical producer set (the SINGLE SOURCE OF TRUTH for this invariant) is the following two scripts: `.claude/features/tdd-state-machine/scripts/tdd-step.py` and `.claude/features/tdd-subagent/scripts/dispatch-tdd-subagent.py`. The mandatory call shape is either `rabbit_block(<named_wrapper>(), ...)` for systemMessage values, or the wrapper output embedded as a string inside an assembled prompt (no `rabbit_block` wrapping when the output IS prompt text). Direct calls to `rabbit_print("message-id", ...)` at producer call sites are forbidden — the named wrappers are the public API for producers. Direct in-line ANSI escape codes (`\x1b[3...`), direct brand-prefix strings (`[🐇 rabbit 🐇]`), bar strings (`━━━`), or leading `"\n"` characters in systemMessage values outside `rabbit_block` are likewise forbidden. The single function `rabbit_block` is the only place the leading newline appears for hook-emitted systemMessages. The rabbit-cage dispatcher hooks (Plan-C model: `stop-dispatcher.py`, `session-start-dispatcher.py`, `user-prompt-submit-dispatcher.py`) are NOT direct producers — they aggregate `print_result` / `banner_result` / `subline_result` returns from `contract.lib.runtime` and route the rendering through `rabbit_print.py` indirectly via the runtime API library, satisfying the same constraint without per-hook `rabbit_print` call sites.
