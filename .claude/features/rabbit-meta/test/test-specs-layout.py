...
End-to-end test verifying rabbit-meta's spec artifacts live under the flat
docs/ layout, the legacy specs/ directory is gone, and the docs/bugs/
subtree is preserved as a sibling:
  ...
  - t5: docs/spec.md frontmatter declares feature: rabbit-meta
  - t6: docs/CHANGELOG.md exists and is non-empty
  - t7: (E2E) the contract resolver resolves spec.md + contract.md at the
        flat docs/ location for rabbit-meta
"""
...
# t5: docs/spec.md frontmatter feature: rabbit-meta
try:
...
if m and m.group(1).strip() == "rabbit-meta":
    ok("t5", "docs/spec.md declares feature: rabbit-meta")
else:
    got = m.group(1).strip() if m else "<missing>"
    fail_t("t5", f"docs/spec.md feature is {got!r}, expected 'rabbit-meta'")
...
# t7: (E2E) the contract resolver targets the flat docs/ location
...
if resolved_spec == expected_spec and resolved_contract == expected_contract:
    ok("t7", "contract resolver targets flat docs/spec.md + docs/contract.md")
else:
    fail_t(
        "t7",
