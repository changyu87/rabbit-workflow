#!/usr/bin/env python3
"""test-bb-vocab-retired.py — issue #665 (part of #420).

End-to-end content test that rabbit-meta's live doc surfaces (docs/spec.md and
docs/contract.md) no longer carry legacy bug-and-backlog ("B/B") LIVE
vocabulary as current prose. The retired bug-and-backlog system was succeeded
by the rabbit-issue system; doc surfaces describe the current design, so they
must speak the current vocabulary ("issue", "rabbit-managed issue", "bug or
enhancement"), NOT the retired "B/B" / "bug-and-backlog" / "bug/backlog" /
standalone "backlog" terms.

This is a regression guard for the prove-it-dead retirement. It scans
rabbit-meta's OWN live doc surfaces line by line, flagging current-usage B/B
prose tokens. A small ALLOWLIST exempts load-bearing literals that legitimately
contain these tokens and CANNOT be reworded without making the surface
inaccurate (the slot mirrors the cross-feature guard in contract).
...
# live prose in rabbit-meta's doc surfaces.
BANNED = [
    r"\bB/B\b",
    r"bug-and-backlog",
    r"bug and backlog",
...
if FAIL > 0:
    print("test-bb-vocab-retired: FAIL", file=sys.stderr)
    sys.exit(1)

print("test-bb-vocab-retired: all checks passed.")
sys.exit(0)
