...
This file holds the tombstones for invariants that were previously declared in `specs/spec.md` and have since been retired. Spec.md carries inline tombstones (one-line `*(Retired — see CHANGELOG.md, <backlog-id>.)*` markers); the full retirement rationale lives here.
