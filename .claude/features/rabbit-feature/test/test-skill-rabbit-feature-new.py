#!/usr/bin/env python3
"""E2E test for rabbit-feature BACKLOG-2.

Asserts the `rabbit-feature-new` skill is present and conforms to spec:
...
def test_frontmatter_required_fields() -> None:
    ...
    assert fm.get("name") == "rabbit-feature-new", (
        f"frontmatter name must be 'rabbit-feature-new', got {fm.get('name')!r}"
    )
...
def test_body_documents_protocol() -> None:
    ...
    assert "validate_feature" in text, (
        "SKILL.md body must reference contract.lib.checks.validate_feature post-scaffold validation"
    )
...
def test_feature_json_surface_lists_skill() -> None:
    ...
    assert "skills/rabbit-feature-new/SKILL.md" in skills, (
        "feature.json surface.skills must list skills/rabbit-feature-new/SKILL.md; "
        f"got {skills}"
    )
