#!/usr/bin/env python3
# test-dispatch-tdd-new-interface.py
import os
import subprocess
import sys
...
result = subprocess.run(['python3', SCRIPT, 'contract', 'test', '--linked-item', '/tmp/fake'],
                        capture_output=True)
if result.returncode != 0:
...
result = subprocess.run(['python3', SCRIPT, 'contract', 'fix the scope guard'],
...
if 'tdd-report.json' in prompt:
    ok("prompt references tdd-report.json")
else:
    fail("prompt missing tdd-report.json")
...
if 'test_gap_analysis' in prompt:
    ok("prompt contains test_gap_analysis")
else:
    fail("prompt missing test_gap_analysis")

# 8. --linked-item --item-type bug is accepted
result2 = subprocess.run(
    ['python3', SCRIPT, 'contract', 'test', '--linked-item', '/tmp/fake-bug', '--item-type', 'bug'],
    capture_output=True, text=True
)
...
import re
if re.search(r'inline|spec.review|spec-review', prompt, re.IGNORECASE):
    ok("prompt mentions inline spec-review")
else:
    fail("prompt missing inline spec-review instruction")
