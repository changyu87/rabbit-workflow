#!/usr/bin/env python3
"""Regression test for the rabbit-spec retirement.

The feature has been retired; its surface (the rabbit-spec skill) was
absorbed into rabbit-feature as rabbit-feature-spec. The directory must
contain only the retirement notice (feature.json plus docs/spec/spec.md)
and this test directory. No scripts/, skills/, or other surface files may
remain.
"""
...
for forbidden in ("scripts", "skills"):
    ...
    if p.exists():
        fail(f"{forbidden}/ still exists under retired rabbit-spec")
...
if unexpected:
    fail(f"unexpected entries in rabbit-spec/: {sorted(unexpected)}")
...
if FAIL:
    print(f"FAILED: {FAIL}")
    sys.exit(1)
print("OK: rabbit-spec retirement notice intact")
