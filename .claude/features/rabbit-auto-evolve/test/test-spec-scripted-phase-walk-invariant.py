...
scripted phase-walk (the dispatcher supplies only Phase 6), not a prose
...
The invariant states:
  (a) ONE shared deterministic phase-walk implementation
      ...
      invoke;
  (b) the in-session path differs ONLY by inserting Phase 6 (dispatch);
  (c) Phase 11 persist re-reads on-disk state, drops the transient
...
REQUIRED_SPEC = [
    ...
    "run-tick-phases.py",
    "scripted phase-walk",
    # (b) in-session differs only by Phase 6 (dispatch).
    "phase 6",
...
if missing_skill:
    ...
    ok("SKILL.md describes the in-session tick via run-tick-phases.py + Phase 6")
