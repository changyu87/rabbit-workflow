#!/usr/bin/env python3
"""test-docs-layout.py — issue #399 Phase 2b (rabbit-decompose flat docs/ move)

End-to-end check of rabbit-decompose's canonical flat-docs/ documentation
layout.

Phase 2 (#399) relocated the feature's spec directory from docs/spec/ to
specs/. Phase 2b (this test) relocates the doc artifacts again, this time
to the flat docs/ layout shared workflow-wide:
...
post-migration on-disk shape independent of any single helper script:
  ...
  - docs/spec.md exists, is non-empty, carries `feature: rabbit-decompose`.
  - docs/contract.md exists, is non-empty, carries `feature: rabbit-decompose`.
  ...
  - docs/spec.md, docs/contract.md, feature.json, and the source
    SKILL.md all declare the same version (four-way alignment survives
    the move).
...
if header_field(spec_text, "feature") != "rabbit-decompose":
    fail("docs/spec.md frontmatter does not declare `feature: rabbit-decompose`")
if header_field(contract_text, "feature") != "rabbit-decompose":
...
skill_text = read_nonempty(skill_md, "skills/rabbit-decompose/SKILL.md")
