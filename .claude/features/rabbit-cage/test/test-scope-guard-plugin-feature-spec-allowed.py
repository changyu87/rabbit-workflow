#!/usr/bin/env python3
"""E2E test for amended Inv 17(a): plugin-mode scope-guard MUST permit
writes to .rabbit/rabbit-project/features/<name>/** when the per-feature
scope-active marker is present.
...
FEATURE_NAME = "rabbit-cage-plugin-feature-allowed-test"
...
def main():
    print("test-scope-guard-plugin-feature-spec-allowed.py")
    print()
    with saved_state():
        ...
        if rc == 0:
            print("PASS: plugin-mode write to "
                  ".rabbit/rabbit-project/features/<name>/** ALLOWED "
                  "when scope-active marker is present")
            return 0
        print(f"FAIL: expected ALLOW (rc=0), got rc={rc} stderr={stderr!r}")
        return 1
