...
# t9: run.sh has at most 10 suite invocations (backlog suite removed, generate-skills-dir suite added)
...
if [ "$suite_count" -le 10 ]; then
    ok 9 "run.sh has $suite_count suite invocations — expected <= 10 after backlog suite removal"
else
    fail_t 9 "run.sh has $suite_count suite invocations — expected <= 10 after backlog suite removal"
fi
