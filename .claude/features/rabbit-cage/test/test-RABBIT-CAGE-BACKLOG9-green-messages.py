...
try:
    ...
    fakesurf = os.path.join(tmp2, ".claude/features/rabbit-cage/test/test-generated-surface.sh")
    ...
    noskills = os.path.join(tmp3, ".claude/features/rabbit-cage/scripts/generate-skills-dir.sh")
    ...
    with open(os.path.join(tmp3b, "CLAUDE.md"), "w") as f:
        f.write("# Rabbit Workflow — test header\n\n@./policy-files/p1.md\n")
    noskills = os.path.join(tmp3b, ".claude/features/rabbit-cage/scripts/generate-skills-dir.sh")
    with open(noskills, "w") as f:
        f.write("#!/usr/bin/env bash\nexit 0\n")
    os.chmod(noskills, 0o755)
...
""")
    ...
    msg = extract_sys_msg(res.stdout)
    assert_green_msg("refresh.py inline-section case", msg)
