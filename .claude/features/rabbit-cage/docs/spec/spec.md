...
18. The Python runtime scripts in rabbit-cage are: in `hooks/` — `refresh.py`, `scope-guard.py`, `session-init.py`, `sync-check.py`; in `scripts/` — `build.py`, `build-targets.py`, `generate-claude-md.py`, `generate-claude-md-header.py`, `new-feature.py`, `rabbit-project.py`, `rabbit-project-consolidate.py`, `rabbit-project-map.py`, `rabbit-project-set-path.py`, `repo-permissions.py`, `scope-guard-on.py`, `workspace-tree.py`. Each preserves the stdin/stdout/exit-code contract of the `.sh` predecessor it replaces. Feature-audit sweeps are owned by the `rabbit-feature-audit` skill in the `rabbit-feature` feature. The `new-feature.py` script remains in rabbit-cage temporarily because `rabbit-feature-new` still resolves `.claude/features/rabbit-cage/scripts/new-feature.py`. Its move into `rabbit-feature` is tracked as the follow-up cycle RABBIT-CAGE-BACKLOG-26.
