...
18. The Python runtime scripts in rabbit-cage are: in `hooks/` — `refresh.py`, `scope-guard.py`, `session-init.py`, `sync-check.py`; in `scripts/` — `build.py`, `generate-claude-md.py`, `generate-claude-md-header.py`, `rabbit-project.py`, `rabbit-project-consolidate.py`, `rabbit-project-map.py`, `rabbit-project-set-path.py`, `repo-permissions.py`, `scope-guard-on.py`, `workspace-tree.py`. Each preserves the stdin/stdout/exit-code contract of the `.sh` predecessor it replaces. Feature-audit sweeps are owned by the `rabbit-feature-audit` skill in the `rabbit-feature` feature. The `new-feature.py` scaffolding script lives in the `rabbit-feature` feature (`.claude/features/rabbit-feature/scripts/new-feature.py`), moved out of rabbit-cage in RABBIT-CAGE-BACKLOG-26.
