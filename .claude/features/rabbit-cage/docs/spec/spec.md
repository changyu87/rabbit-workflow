...
- `.claude/skills/` — directory of per-skill `SKILL.md` copies (one `SKILL.md` file per skill, per `build-contract.json` `copy-file` targets); subdirs like `scripts/` are NOT deployed and remain at their canonical source path; committed to the repo
...
18. The Python runtime scripts in rabbit-cage are: in `hooks/` — `refresh.py`, `scope-guard.py`, `session-init.py`, `sync-check.py`; in `scripts/` — `build.py`, `build-targets.py`, `generate-claude-md.py`, `generate-claude-md-header.py`, `rabbit-project.py`, `rabbit-project-consolidate.py`, `rabbit-project-map.py`, `rabbit-project-set-path.py`, `repo-permissions.py`, `scope-guard-on.py`, `workspace-tree.py`. Each preserves the stdin/stdout/exit-code contract of the `.sh` predecessor it replaces. Feature-audit sweeps are owned by the `rabbit-feature-audit` skill in the `rabbit-feature` feature. The `new-feature.py` scaffolding script lives in the `rabbit-feature` feature (`.claude/features/rabbit-feature/scripts/new-feature.py`), moved out of rabbit-cage in RABBIT-CAGE-BACKLOG-26.
...
Manages the Step 4 (HUMAN-APPROVAL) bypass state via the marker file `.rabbit-human-approval-bypass` at the repo root. The marker is gitignored. The subcommand takes a boolean value following contract Inv 11 (boolean CLI values use `true`/`false`) where `true` = human approval gate ACTIVE (marker absent, dispatcher waits) and `false` = gate OFF / bypass ACTIVE (marker written, dispatcher skips Step 4). This naming optimizes for natural-language operator instructions like "turn off human approval" — the subcommand value answers "is human approval in effect?" with the literal `true`/`false`:

- `/rabbit-config human-approval false` — writes `.rabbit-human-approval-bypass` at the repo root with content `session`. The Step 4 bypass is ACTIVE: every subsequent `rabbit-feature-touch` dispatch passes `--human-approval-gate false` to `dispatch-tdd-subagent.py` and skips the in-conversation wait. Use ONLY after explicit user authorization. Natural-language mapping: "turn off human approval" → `human-approval false` → marker WRITTEN.
- `/rabbit-config human-approval true` — deletes `.rabbit-human-approval-bypass`. The Step 4 bypass is OFF and the human-approval gate is back in force: `rabbit-feature-touch` dispatchers wait for explicit in-conversation user approval. This is the default posture for a fresh checkout. Natural-language mapping: "turn on human approval", "re-enable approval" → `human-approval true` → marker REMOVED.
- `/rabbit-config human-approval` (no action) — prints the current gate state to stdout: either `true` (marker absent, gate active) or `false` (marker present, bypass active). No file is modified.
...
**Why this differs from `bypass-permissions`.** `bypass-permissions` is pinned to Claude Code's own config value `"bypassPermissions"`; its naming is externally constrained and uses `true`=bypass-ACTIVE. `human-approval` is internally controlled, and optimizes for natural-language clarity instead of cross-subcommand parallelism. The two subcommands deliberately use opposite boolean conventions because the cost of a misparsed `bypass-X` (auto-running with no approval) is asymmetric with the cost of a misparsed `human-approval` (waiting for approval that already happened).

**Legacy subcommand name removed:** the prior subcommand name `bypass-human-approval` (introduced by RABBIT-CAGE-BACKLOG-31 with semantics where `true` meant "bypass ACTIVE") is REMOVED by RABBIT-CAGE-BUG-97. The dispatcher rejects `/rabbit-config bypass-human-approval` (any form) with a non-zero exit and an error message directing the operator to the new `human-approval` spelling with the inverted boolean. No coexistence window is offered: this is a hard rename. The reason for the revert is a natural-language misparse — "turn off human approval" was ambiguously read as "turn off [the bypass-human-approval subcommand]" → `false`, which under bypass-X semantics meant gate ACTIVE, opposite of operator intent. Re-aligning with the original `human-approval` spelling makes the boolean read literally as "is human approval in effect?", which disambiguates the conversational instruction.

### Subcommand: bypass-permissions

Manages the per-user `permissions.defaultMode = "bypassPermissions"` key in `.claude/settings.local.json`. Bypass mode tells Claude Code to skip its native per-write permission prompts so the scope-guard PreToolUse hook becomes the single decision point for write authorization. Because different operators may want different behavior, this is a per-user opt-in — it lives in `settings.local.json` (gitignored), never in the shared `settings.json`. The subcommand takes a boolean value following contract Inv 11 (boolean CLI values use `true`/`false`):
...
37. `/rabbit-config human-approval` (no action) prints exactly one line to stdout: `true` if `.rabbit-human-approval-bypass` is ABSENT at repo root (gate active), otherwise `false` (marker present, bypass active). No file is modified. Exits 0. The state-query value follows the "is human approval in effect?" convention — opposite of the `bypass-permissions` true=bypass-active convention, deliberately, so that natural-language "turn off human approval" maps cleanly to `human-approval false`.
38. `.rabbit-human-approval-bypass` is gitignored (appears in `.gitignore`). The marker is a runtime artifact, never committed. The marker filename is intentionally unchanged across both the BACKLOG-31 rename and the BUG-97 revert — it describes what the marker IS (a bypass record), not the subcommand that toggles it.
...
41. `session-init.py` MUST NOT auto-create or auto-switch git branches. Operators are responsible for creating their own feature branches before editing; direct commits to `main` remain blocked by the `Bash(git push * main)` deny rules (Inv 19) and by the `check-no-main-edits.py` enforcement script in contract.
...
46. (Retired in RABBIT-CAGE-BACKLOG-26 — `new-feature.py` has moved to the `rabbit-feature` feature; its scaffolding contract is owned by rabbit-feature's spec.)
...
48. `rabbit-config.py human-approval <true|false>` confirmation message MUST be self-explanatory and name the literal marker file. The `false` form MUST state that the bypass is ACTIVE and the marker was written, e.g., `Human approval gate DISABLED (bypass ACTIVE). Marker .rabbit-human-approval-bypass written. Step 4 will be skipped for all dispatches until you run /rabbit-config human-approval true.` Conversely `true` MUST say `Human approval gate ENABLED. Marker .rabbit-human-approval-bypass removed. Step 4 will wait for in-conversation approval on each dispatch.` Idempotent invocations MUST state that the file/marker was not rewritten (e.g., `Human approval gate already DISABLED (marker .rabbit-human-approval-bypass already present; no rewrite). …`). The confirmation MUST NOT use the bare verbs ENABLED/DISABLED without naming what is being enabled — the operator should not have to remember which side of the boolean is which. The framing names the "gate" (what `true` activates) and explicitly notes when bypass is "ACTIVE" (what `false` activates), so the operator can read the confirmation cold and know what state the system is in.
...
62. `session-init.py` MUST emit a status-flags block in its startup banner that lists every active runtime override at session start, so a fresh session begins with full visibility into the active configuration. The block MUST include one line per active flag for: (a) human-approval bypass (`.rabbit-human-approval-bypass` exists at repo root); (b) bypass-permissions mode (`.claude/settings.local.json` declares `permissions.defaultMode == "bypassPermissions"`); (c) any future runtime override added to rabbit-cage via the same per-user-marker pattern. Each line MUST use the canonical `[🐇 rabbit 🐇]` brand prefix (per contract Inv 28/29), MUST carry a leading icon between the brand prefix and the body text (`🔑` for human-approval bypass, `🚨` for bypass-permissions mode — sourced from the per-flag registry entries in `rabbit-print-messages.json` per contract Inv 27, surfaced through `rabbit_subline(..., icon=...)` per contract Inv 28(b)), and MUST name both the active condition and the canonical revoke command (`/rabbit-config <subcommand> <value>`). The leading icon distinguishes alert-bearing sublines from icon-less policy-info sublines so the operator's eye is drawn to active overrides at the same visual weight as the Stop-event alerts emitted by Inv 61. When no flags are active, the block is omitted entirely (baseline banner is unchanged — no empty header, no "all clear" affirmation, so the banner stays terse when nothing is overridden). The startup-banner emission is parallel to but distinct from the Stop-event emission in Inv 39 / Inv 61: startup-time is a one-shot orientation; Stop-time is a recurring per-event reminder. Both share the SAME canonical message text per flag so operators see one consistent line whether the session is starting or stopping.
...
73. `[🐇 rabbit 🐇]` `systemMessage` brand, decoration, color, and message
    ...
    (CONTRACT-BACKLOG-20 / contract Inv 4, 27, 28, 29). Rabbit-cage hooks
...
74. The scope guard recognizes two coexisting scope-marker formats at the
    ...
    feature name; serial-dispatch form) and per-feature markers
...
91. The interim `/rabbit-config bypass-human-approval` subcommand name
    (introduced by RABBIT-CAGE-BACKLOG-31 with semantics where `true`
    meant "bypass ACTIVE") is REMOVED by RABBIT-CAGE-BUG-97 in favor
    of restoring the original `human-approval` spelling (semantics:
    `true` = gate ACTIVE / marker absent, `false` = bypass ACTIVE /
    marker written). The hard rename has no coexistence window. Invoking
    `/rabbit-config bypass-human-approval [...]` (any form, with or
    without arguments) MUST exit non-zero without modifying any file
    and MUST print an error message to stderr that explicitly names
    the restored spelling `human-approval` and the restored boolean
    semantics so operators can migrate without consulting the spec.
    The generic unknown-subcommand error (per Inv 24) is NOT sufficient
    here because the interim name carries a specific migration path
    that deserves a directed message; this invariant strengthens Inv 24
    for the `bypass-human-approval` literal. Cross-feature documentation,
    hook canonical-revoke text, and contract surface declarations
    referring to `/rabbit-config bypass-human-approval ...` MUST be
    updated to the `human-approval` form with the restored boolean
    value (e.g., the revoke command for an active human-approval bypass
    is `/rabbit-config human-approval true`, not `/rabbit-config
    bypass-human-approval false`). The reason for the revert is captured
    in Inv 95: a natural-language misparse of "turn off X" where X
    itself led with a negating verb.
...
95. **Natural-language regression pin (RABBIT-CAGE-BUG-97).** The conversational instruction "turn off human approval" MUST map to `/rabbit-config human-approval false`, which in turn MUST write the marker file `.rabbit-human-approval-bypass` (bypass ACTIVE). Symmetrically, "turn on human approval" / "re-enable approval" MUST map to `/rabbit-config human-approval true`, which MUST delete the marker (bypass OFF / gate ACTIVE). A regression test MUST live at `.claude/features/rabbit-cage/test/test-RABBIT-CAGE-BUG-97-natural-language-mapping.py` and MUST be wired into `test/run.py`. The test asserts BOTH directions of the mapping by invoking `rabbit-config.py` as a subprocess in an isolated temp workspace (per Inv 44) and inspecting the marker file's presence/absence after each invocation. The test is belt-and-suspenders against future re-rename attempts that would re-introduce the natural-language misparse documented in Inv 91; a failure of this test is a hard signal that the subcommand naming has drifted away from the natural-language contract that motivated the BUG-97 revert.
