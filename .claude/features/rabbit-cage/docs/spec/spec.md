...
22. **Install-time `--update` mode for in-place refresh.** `install.py main()` accepts an optional `--update` flag. `install.sh` accepts the same flag at the shell layer (and honours `RABBIT_UPDATE=true` as an env-var equivalent) and forwards it verbatim to `install.py`. Update-mode semantics:
    ...
    (e) **Version visibility.** Before refresh, when `<target>/.version` exists, `install.py main()` reads its content and prints `updating <old-ref> -> <new-ref>` to stdout (where `<new-ref>` is the value `write_version_pin` is about to write — `RABBIT_INSTALLED_REF` env var, default `"unknown"`). The existing `.version` file is reused; no new state file is introduced.
