...
- **v5.94.1 (enh #1209: add `resolve-project-remote.py` to FEATURE_INCLUDES["rabbit-housekeep"] closure):** rabbit-housekeep v0.9.1 (PR #1208/#1206) added a new script `scripts/resolve-project-remote.py` (resolves the consuming project's GitHub remote to an `owner/repo` slug for sub-issue routing). The script was declared in rabbit-housekeep's `feature.json` surface.scripts and `docs/contract.md` provides.scripts but absent from `install.py`'s `FEATURE_INCLUDES["rabbit-housekeep"]` closure -- so a fresh `curl ... install.sh | bash` / vendored install would omit it, causing runtime failure if any governed step invokes it via a full literal path. FIX: `scripts/resolve-project-remote.py` added to `FEATURE_INCLUDES["rabbit-housekeep"]`, and the pinned authoritative ship set `EXPECTED_SCRIPTS` in `test/test-install-closure-ships-rabbit-housekeep.py` is extended to match (the generic Inv 24 / #897 / #1035 tests enforce closure completeness for SHIPPED surfaces; this pinned test guards against the feature entry being dropped). `install.py` is a `publish_file` destination -- its deployed repo-root copy drifts until the dispatcher republishes; `install.py` module docstring Version bumped 6.12.0 -> 6.12.1. Feature/spec/contract versions bumped 5.94.0 -> 5.94.1 in lockstep (Inv 11). Closes #1209.
- **v5.94.0 (fix #1193: the reload-tier restart advisory is now clearable via `/rabbit-refresh`, not the dead `/reload-skills` path):** the #1173 fix wired the reload-tier advisory's clear to a `/reload-skills` SUBMITTED prompt in `hooks/user-prompt-submit-dispatcher.py` — but `/reload-skills` is a Claude Code CLIENT-LOCAL built-in that reloads `SKILL.md` definitions in-process and NEVER fires the UserPromptSubmit hook (it surfaces only as a local command with its own `Reloaded skills: N available` stdout, never as a model-bound prompt). So the dispatcher never observed `/reload-skills`, `rebaseline_skill_tier` was never called, and the `reload ADVISED` advisory persisted until a full restart — the exact remedy it advised against. That prompt-match branch (and the now-orphaned `_submitted_prompt` helper + `_RELOAD_SKILLS_COMMAND`) is DEAD and is removed. FIX: the re-baseline now lives on the `/rabbit-refresh` command, which IS a real submitted command whose body runs deterministic `!` bash (script-tier per spec-rules §1). New companion script `scripts/rabbit-refresh-rebaseline.py` calls `hooks/restart_snapshot.py`'s `rebaseline_skill_tier` (refreshes ONLY the `.claude/skills/**/SKILL.md` snapshot keys, leaving the hard-restart tier untouched); `commands/rabbit-refresh.md`'s body invokes it (command version 1.0.0 -> 1.1.0). The reload advisory text now directs the user to run `/reload-skills` to load the change AND `/rabbit-refresh` to dismiss the advisory. A NEW `SKILL.md` change after a re-baseline still fires a fresh advisory (the clear is a re-baseline of current state, not a permanent suppression). Inv 54 clause (f) rewritten to the corrected mechanism; the dead-path test `test/test-reload-skills-clears-reload-advisory.py` (#1173) is replaced by `test/test-rabbit-refresh-clears-reload-advisory.py` (e2e: real deployed dispatchers + the real companion script run as a subprocess) — `run.py` re-wired. The new script is added to `install.py`'s `FEATURE_INCLUDES['rabbit-cage']` closure (Inv 24 deployed-surface-referenced-script gate); `install.py` module docstring Version bumped 6.11.1 -> 6.12.0. `restart_snapshot.py` module Version bumped 1.4.0 -> 1.5.0. `user-prompt-submit-dispatcher.py`, `restart_snapshot.py`, `rabbit-refresh.md`, and `install.py` are deployed surfaces — their deployed copies drift until the dispatcher republishes. Feature/spec/contract versions bumped 5.93.1 -> 5.94.0 in lockstep (Inv 11). Closes #1193.
- **v5.93.1 (fix #1191/#1195: install closure now ships rabbit-housekeep's `wave-automerge.py`):** rabbit-housekeep's shipped `SKILL.md` (#1191) added 3 literal references to `scripts/wave-automerge.py` (the SCRIPT-tier gated wave auto-merge backing script), but `install.py`'s `FEATURE_INCLUDES['rabbit-housekeep']` did not list it — so a fresh `curl … install.sh | bash` / vendored install omitted the script and the deployed `SKILL.md` would fail at runtime with "No such file or directory" (the #897/#1035 deployed-surface-referenced-script regression class). FIX is additive closure-only: `FEATURE_INCLUDES['rabbit-housekeep']` gains `scripts/wave-automerge.py`, and the pinned authoritative ship set in `test/test-install-closure-ships-rabbit-housekeep.py` (`EXPECTED_SCRIPTS`) is extended to match. This makes both generic closure gates GREEN: `test/test-install-ships-skill-referenced-scripts.py` (#897/#1035) and `test/test-feature-includes-scripts-closure.py` (Inv 24, case t20). rabbit-housekeep's half (the script + SKILL.md Step 8) lands on the same shared branch from the first subagent; this commit is the rabbit-cage closure half (Inv 26 multi-subagent-barrier). `install.py` is a `publish_file` destination — its deployed repo-root copy drifts until the dispatcher republishes; `install.py` module docstring Version bumped 6.11.0 -> 6.11.1. Feature/spec/contract versions bumped 5.93.0 -> 5.93.1 in lockstep (Inv 11). Closes #1195; part of #1191.
- **v5.93.0 (fix #1181: install.py closure now ships rabbit-housekeep — skill/command/scripts were absent in vendored installs):** `install.py`'s hardcoded vendored-install file closure (`SKILLS`, `COMMANDS`, `FEATURE_INCLUDES`) entirely OMITTED rabbit-housekeep, so a fresh `curl … install.sh | bash` / plugin install shipped NO rabbit-housekeep `SKILL.md`, no `/rabbit-housekeep` command, no `feature.json`, and no backing scripts — the root cause of #1179. The user-facing command + scope resolver added at SOURCE by #1180 were invisible to every vendored install. FIX adds the three closure entries: `SKILLS` gains `skills/rabbit-housekeep/SKILL.md → .claude/skills/rabbit-housekeep/SKILL.md`; `COMMANDS` gains `commands/rabbit-housekeep.md → .claude/commands/rabbit-housekeep.md`; `FEATURE_INCLUDES['rabbit-housekeep']` ships `feature.json`, the `SKILL.md`, the command `.md`, and the three backing scripts the deployed surface references (`scripts/measure-reduction.py`, `scripts/check-script-backed.py`, `scripts/resolve-housekeep-scope.py`). The script set is derived from the literal `.claude/features/rabbit-housekeep/scripts/*.py` references in the shipped `SKILL.md` + command bodies (the #897/#1035 deployed-surface-referenced-script gate and the Inv 24 feature-includes-scripts-closure test); `docs/` and `test/` are development surfaces and are excluded per the closure convention. The neutralized prose mention of rabbit-auto-evolve's loop-only `record-decomposition.py` / `close-decomposed-parents.py` (#1182) carries no shipped-path invocation, so the closure stays self-contained — no dangling required-file. New regression test `test/test-install-closure-ships-rabbit-housekeep.py` pins the positive expectation (the generic closure tests only scan SHIPPED surfaces and cannot observe a wholly-absent feature); `run.py` wired. `install.py` is a `publish_file` destination — its deployed repo-root copy drifts until the dispatcher republishes; `install.py` module docstring Version bumped 6.10.0 -> 6.11.0. Feature/spec/contract versions bumped 5.92.0 -> 5.93.0 in lockstep (Inv 11). Closes #1181.
- **v5.92.0 (fix #1173: the `/reload-skills` advisory now clears once the reload actually runs):** the tiered `/reload-skills` advisory added in #1156 (Inv 54e) never cleared after the user ran `/reload-skills`, so it nagged on every Stop / UserPromptSubmit forever. Root cause: clause (b) re-baselines the snapshot ONLY on SessionStart, and `/reload-skills` is a Claude Code built-in that reloads `SKILL.md` definitions mid-session WITHOUT starting a session — so it never updated rabbit-cage's snapshot, defeating the advisory's own "no restart needed" promise. FIX: `hooks/user-prompt-submit-dispatcher.py` now inspects the SUBMITTED prompt text (the UserPromptSubmit stdin JSON `prompt` field, which it previously read and discarded); when the prompt is exactly `/reload-skills`, it calls the new `hooks/restart_snapshot.py` helper `rebaseline_skill_tier` BEFORE the clause-(c)/(e) re-check. That helper refreshes ONLY the `.claude/skills/**/SKILL.md` keys of the snapshot to their current on-disk sha256 (dropping any vanished `SKILL.md` key) and leaves every non-`SKILL.md` key — the hard-restart tier (hooks / settings / `CLAUDE.md` / agents) and the skill-`scripts/` keys — exactly as the SessionStart baseline recorded them. So the reload-tier advisory clears on the reload tick and stays cleared, while a genuine hook/settings/`CLAUDE.md`/agent change present at the same time STILL fires the full-restart advisory. Best-effort/graceful as clause (d): no snapshot, an empty/unparseable stdin payload, a non-`/reload-skills` prompt, or any IO error is a silent no-op. Inv 54 extended with clause (f). New e2e test `test/test-reload-skills-clears-reload-advisory.py` (real deployed dispatchers, stdin prompt payload): a `/reload-skills` prompt clears the reload advisory on its own emission and keeps it cleared on the next tick; a concurrent hook change's restart advisory survives the reload and persists; an unrelated prompt leaves the reload advisory firing. `run.py` wired. `restart_snapshot.py` and `user-prompt-submit-dispatcher.py` are deployed `publish_file` surfaces — their `.claude/hooks/` copies drift until the dispatcher republishes. `restart_snapshot.py` module Version bumped 1.3.0 -> 1.4.0. Feature/spec/contract versions bumped 5.91.0 -> 5.92.0 in lockstep (Inv 11). Closes #1173.
- **v5.91.0 (enh #1156: tier the mid-session restart advisory by change type — recommend `/reload-skills` for `SKILL.md`-only changes):** the Inv 54 mid-session advisory emitted a blanket `restart ADVISED` line for ANY on-disk change to a restart-sensitive surface. Since Claude Code v2.1.152, `/reload-skills` reloads `SKILL.md` skill definitions mid-session WITHOUT a restart, and skill `scripts/` are re-read on every invocation, so a full restart is overkill for those cheaper change types. `restart_snapshot.py`'s `restart_advisory_payloads` now classifies the SET of changed restart-sensitive paths and emits the CHEAPEST sufficient remedy: ONLY `.claude/skills/**/SKILL.md` changed → a `/reload-skills` advisory (`reload ADVISED (not required): skill definitions changed on disk — run /reload-skills to load the update (no restart needed)`, icon 🔄, color yellow, same `(as of HH:MM:SS ZZZ)` timestamp); ONLY skill `scripts/` changed → NOTHING (re-read on next call); any hook/`CLAUDE.md`/settings/`.claude/agents/**` change → the full-restart advisory of clause (c), unchanged; MIXED → the superset wins (strict precedence full-restart > reload-skills > nothing). Classification is pure path inspection over the changed-key set, best-effort/graceful as clause (d). Inv 54 extended with clause (e). New e2e test `test/test-restart-advisory-tiered-by-change-type.py` (real deployed dispatchers): `SKILL.md`-only → `/reload-skills` not full-restart on BOTH Stop and UserPromptSubmit; skill-`scripts/`-only → neither advisory; hook change → full-restart not `/reload-skills`; mixed `SKILL.md`+hook → full-restart (superset); mixed `SKILL.md`+skill-`scripts/` → `/reload-skills`. The pre-existing `test/test-restart-advisory-on-disk-change.py` skill case is updated from the full-restart fragment to the tiered `/reload-skills` fragment; the hook/settings/`CLAUDE.md`/agent cases are unchanged. `run.py` wired. `restart_snapshot.py` is a deployed `publish_file` surface — its `.claude/hooks/` copy drifts until the dispatcher republishes. Feature/spec/contract versions bumped 5.90.0 -> 5.91.0 in lockstep (Inv 11). Closes #1156.
- **v5.90.0 (fix #1152: vendored restart-needed marker is now gitignored):** in vendored/Strategy-D mode `<rabbit_root>` IS the fully-tracked `.rabbit/` install dir, so `scripts/rabbit-update.py`'s restart-needed marker landed at `.rabbit/.rabbit-update-restart-needed` — inside the tracked tree and NOT covered by the `.runtime/` ignore entry — surfacing as an untracked file in the host repo after every restart-sensitive self-update (observed in ws2 after both v10.34.6 and v10.34.9). The marker is ephemeral runtime state (written by `install`, read + consumed once by contract's `check_release_update` per contract Inv 39, never committed). FIX is gitignore-coverage only: `.rabbit-update-restart-needed` is added to the inner `.rabbit/.gitignore` source of truth `_RABBIT_GITIGNORE_BODY` (rendered by `write_rabbit_gitignore`), so a vendored host repo ignores it and `untrack_ignored_rabbit_ephemerals` auto-untracks any copy a pre-fix host already committed on the next `--update` (Inv 53(e), the exact convergence the issue needs). The marker PATH is UNCHANGED (`<rabbit_root>/.rabbit-update-restart-needed`, matching contract's reader byte-for-byte) — neither rabbit-update.py's WRITE side nor contract's READ side is touched; standalone mode is governed by Inv 9's repo-root `.gitignore` and is unaffected. Inv 35(b) extended to document the vendored gitignore coverage. New e2e assertion `test/test-rabbit-update-restart-marker.py` r6 (real `write_rabbit_gitignore` + real `git check-ignore` under a vendored `.rabbit/` root): `.rabbit-update-restart-needed` is in `_RABBIT_GITIGNORE_BODY` and the rendered inner gitignore ignores `.rabbit/.rabbit-update-restart-needed`. `install.py` is a `publish_file` destination — its deployed repo-root copy drifts until the dispatcher republishes. Feature/spec/contract versions bumped 5.89.0 -> 5.90.0 in lockstep (Inv 11). Closes #1152.
- **v5.89.0 (enh #1137: `install.py --update` auto-untracks newly-ignored `.rabbit/` ephemerals):** follow-up from #1135. `write_rabbit_gitignore(dst_root)` writes the inner `.rabbit/.gitignore` ephemeral ignore list, which grows as new ephemerals are recognized (e.g. `.rabbit-prompt-counter` in #1135). A gitignore entry does NOT untrack a file an existing vendored host repo committed BEFORE the entry was added, so under Strategy D full-vendor those installs keep churning the now-ignored file every session until a human runs `git rm --cached`. NEW exported helper `untrack_ignored_rabbit_ephemerals(dst_root)` self-heals on `--update`: against the host repo (`dst_root.parent`) it index-only untracks (`git rm --cached` — file STAYS on disk, never a plain `git rm`) every path that BOTH is TRACKED under `.rabbit/` AND matches the current inner ignore list. The match set is derived from the SAME source of truth `write_rabbit_gitignore` renders — `git check-ignore` over the just-written `.rabbit/.gitignore` (the inner-ignore body is factored into the shared module constant `_RABBIT_GITIGNORE_BODY` so the list lives in one place) — so gitignore glob semantics are never reimplemented and no second copy of the ephemeral names is hardcoded. Wired into `main()` AFTER `write_rabbit_gitignore`/`write_host_gitignore` and before `write_version_pin`, gated to `--update`. Additive: `write_rabbit_gitignore`'s output and the host-gitignore management are unchanged. Clean no-op (never a failure mode) when standalone (dst_root not a `.rabbit` dir), when nothing tracked matches, when `.rabbit/` is untracked, when not a git repo, or when git is unavailable; only matching tracked files are touched. Spec Inv 53 extended with clause (e). New e2e test `test/test-install-update-untracks-ignored-ephemerals.py` (real helper + real git repo): a vendored host that already TRACKS an inner-ignored ephemeral plus a tracked non-ephemeral → ephemeral untracked yet remains on disk, non-ephemeral untouched; an all-non-ephemeral tracked set → no-op; standalone dst_root and non-repo dst_root → clean no-op. `run.py` wired. `install.py` module docstring Version bumped 6.8.0 -> 6.9.0. `install.py` is a `publish_file` destination — its deployed repo-root copy drifts until the dispatcher republishes. Feature/spec/contract versions bumped 5.88.3 -> 5.89.0 in lockstep (Inv 11). Closes #1137.
- **v5.88.3 (fix #1135: inner `.rabbit/.gitignore` now ignores `.rabbit-prompt-counter`):** `install.py`'s `write_rabbit_gitignore(dst_root)` writes the vendored inner `.rabbit/.gitignore` ephemeral list and listed `.rabbit-restart-snapshot` but OMITTED its sibling per-session ephemeral `.rabbit-prompt-counter` — the counter the dispatcher hooks rewrite each session (declared in `feature.json` runtime config and stripped from the workspace tree by `scripts/workspace-tree.py`). Under Strategy D full-vendor a host repo tracks the whole `.rabbit/` tree, so the un-ignored counter got committed and then churned the working tree every session (observed: a runtime-modified `.rabbit/.rabbit-prompt-counter` blocked `git checkout main` after a PR merge on v10.34.1). This is impl falling short of spec Inv (every runtime marker written under the repo root MUST be listed in `.gitignore`, mandated set includes `.rabbit-prompt-counter`), so the FIX brings the installer into compliance — no spec change needed: `.rabbit-prompt-counter` is added to the ephemeral list adjacent to `.rabbit-restart-snapshot`. New e2e test `test/test-rabbit-gitignore-includes-prompt-counter.py` (real helper) asserts the generated inner `.gitignore` lists `.rabbit-prompt-counter` alongside `.rabbit-restart-snapshot`; `run.py` wired. `install.py` is a `publish_file` destination — its deployed repo-root copy drifts until the dispatcher republishes. The one-time `git rm --cached .rabbit/.rabbit-prompt-counter` for already-vendored installs is an operational step, NOT part of this source fix. Feature/spec/contract versions bumped 5.88.2 -> 5.88.3 in lockstep (Inv 11). Closes #1135.
- **v5.88.2 (fix #1124: mid-session restart advisory line now renders in yellow):** the Inv 54 mid-session `restart ADVISED (not required)` line surfaced by the Stop / UserPromptSubmit dispatchers (via `hooks/restart_snapshot.py`) printed in GREEN, so the advisory read as ordinary status output and was easy to miss. It should signal advisory/warning level. `restart_snapshot.py`'s `restart_advisory_payloads` now emits the advisory `print` payload with `color: yellow` instead of `green`, so the line renders in yellow via the shared `rabbit_print` color mechanism (the established green/red/yellow palette `_dispatcher_lib.render_emission` feeds through `rabbit_subline`) — advisory severity distinct from a hard red error. The advisory wording, icon (🔄), timestamp (from #1123), and trigger logic are otherwise unchanged; only the payload `color` field changed. Inv 54(c) spec prose and its enforcement clause extended (new (xi)); new e2e assertion in `test/test-restart-advisory-on-disk-change.py` checks the surfaced line carries the yellow ANSI color marker on both the Stop and UserPromptSubmit paths. `restart_snapshot.py` is a deployed `publish_file` surface — its `.claude/hooks/` copy drifts until the dispatcher republishes. Feature/spec/contract versions bumped 5.88.1 -> 5.88.2 in lockstep (Inv 11). Closes #1124.
- **v5.88.1 (fix #1123: mid-session restart advisory line now carries a wall-clock timestamp):** the Inv 54 mid-session `restart ADVISED (not required)` line surfaced by the Stop / UserPromptSubmit dispatchers (via `hooks/restart_snapshot.py`) printed WITHOUT any timestamp, so a reader could not judge whether the alert was current or stale — every other rabbit hook output (notably the universal Stop turn-end marker, Inv 57) shows when the event fired. `restart_snapshot.py`'s `restart_advisory_payloads` now appends ` (as of HH:MM:SS ZZZ)` to the advisory text, rendered in local wall-clock with the same `%H:%M:%S %Z` format the Stop turn-end timestamp uses, so freshness is judgeable. The advisory wording, icon (🔄), color, and trigger logic are otherwise unchanged; the timestamp is appended after the existing message. Inv 54(c) spec prose and its (x) enforcement clause extended; new e2e assertion in `test/test-restart-advisory-on-disk-change.py` checks the surfaced line matches `(as of HH:MM:SS ZZZ)`. `restart_snapshot.py` is a deployed `publish_file` surface — its `.claude/hooks/` copy drifts until the dispatcher republishes. Feature/spec/contract versions bumped 5.88.0 -> 5.88.1 in lockstep (Inv 11). Closes #1123.
- **v5.88.0 (fix #1117: mid-session restart advisory for ANY on-disk change to a loaded restart-sensitive surface — every update path, not just `/rabbit-update install`):** updating rabbit's session-loaded artifacts (`.claude/skills/`, `.claude/agents/`, and the existing hooks/settings/`CLAUDE.md` set) via a `git pull`/merge, manual edit, or IDE sync left a running session silently holding the stale copies with NO restart advisory — rabbit's counter-drift promise was honored only on the `/rabbit-update install` path that writes `.rabbit-update-restart-needed`. This is a SPEC GAP (the surface set was narrowed and the read was gated to SessionStart behind the upstream-release probe), not an impl-vs-spec mismatch, so the spec is EXTENDED. New Inv 54: a marker-free, deterministic snapshot/re-check owned by rabbit-cage. SessionStart writes a content-hash signature of the restart-sensitive surface set — every file under `.claude/hooks/`, `.claude/skills/`, `.claude/agents/`, every `.claude/settings*.json`, and root `CLAUDE.md` — to `<repo_root>/.rabbit-restart-snapshot` (the loaded baseline); the Stop and UserPromptSubmit dispatchers recompute the signature and, on a mismatch, surface ONE `restart ADVISED (not required)` line (icon 🔄), persisting across tick-ends until a fresh SessionStart re-baselines. New shared helper `hooks/restart_snapshot.py` (`restart_sensitive_signature` / `write_session_snapshot` / `restart_advisory_payloads`), deployed alongside `hooks/_dispatcher_lib.py` via a new `publish_file` manifest entry and listed in `install.py` HOOKS + `FEATURE_INCLUDES["rabbit-cage"]`. Inv 35(b)'s `install`-path surface set widened to the SAME skills/agents set so both paths agree on what is restart-sensitive (`scripts/rabbit-update.py` `RESTART_SENSITIVE_DIRS` extended). Independent of contract's `check_release_update` marker reader (Inv 39) and rabbit-auto-evolve's loop advisory (Inv 37) — fires WITHOUT either marker. New e2e test `test/test-restart-advisory-on-disk-change.py` (real deployed dispatchers as subprocesses): each restart-sensitive surface — hook, settings, `CLAUDE.md`, `SKILL.md`, agent `.md` — changed individually via a direct edit (non-`install` path, neither marker present) triggers the advisory on BOTH Stop and UserPromptSubmit; an unchanged tree and a non-sensitive docs change trigger none; a fresh SessionStart re-baselines so the advisory clears after a restart. `run.py` wired. New deployed surface `restart_snapshot.py` and the three dispatchers drift until the dispatcher republishes. Feature/spec/contract versions bumped 5.87.0 -> 5.88.0 in lockstep (Inv 11). Closes #1117.
- **v5.87.0 (Inv 25 follow-on to contract #1113: vendored scope-override alert now fires via the `.rabbit/`-prefixed entry):** contract #1113 changed `contract.lib.runtime.check_marker_alert` to resolve a repo-root marker against the GIT TOPLEVEL (markers root — `dirname(repo_root)` when vendored) so rabbit-feature's git-toplevel-written `.rabbit-tdd-autonomous` Stop/SessionStart alert fires in a vendored install. rabbit-cage's session-override marker lives INSIDE `.rabbit` in plugin/vendored mode (`<rabbit_root>/.rabbit-scope-override`, Inv 25), so under the new resolution it is the explicit `.rabbit/`-prefixed Inv 25 entry (entry 5 — `{path: '.rabbit/.rabbit-scope-override'}`) that resolves back into the install dir and fires the SCOPE GUARD OFF banner in vendored mode (joining the toplevel with `".rabbit/.rabbit-scope-override"` yields `<repo_root>/.rabbit-scope-override`); the bare entry (entry 4) serves the standalone toplevel location. The user-visible alert is unchanged in BOTH modes — both entries were already declared on Stop and SessionStart (Inv 25), and the full dispatcher-chain tests are green. ONLY the white-box unit test `test/test-plugin-stop-alert-fires-when-session-override-active.py` was over-pinned to the OLD bare-path-resolves-against-`repo_root` mechanism; it is updated to call `check_marker_alert` with the `.rabbit/`-prefixed plugin path (and a sibling host entry so `detect_mode` reads vendored) and stays green. Inv 25 SessionStart-dispatch prose updated to state the markers-root resolution and why both the bare and `.rabbit/`-prefixed paths are declared. No runtime/source behavior change in rabbit-cage — test + spec maintenance driven by the contract API fix. Feature/spec/contract versions bumped 5.86.0 -> 5.87.0 in lockstep (Inv 11).
- **v5.86.0 (enh #1099: `/rabbit-update install` sets the restart-needed marker contract's banner reads):** the WRITE side of contract's #1096 SessionStart restart alert. #1096 (merged) made `contract.lib.runtime.check_release_update` READ + consume the marker `<repo_root>/.rabbit-update-restart-needed` to append a "RESTART REQUIRED" banner line; nothing was WRITING it. `scripts/rabbit-update.py`'s `cmd_install` now, after a SUCCESSFUL `install.py --update` (exit zero), diffs a content-hash signature of the RESTART-SENSITIVE surfaces — the `.claude/hooks/` tree, any `.claude/settings*.json`, and root `CLAUDE.md` — taken before vs after the update; when the signature changed it writes the marker so the next session alerts the user to restart. A no-change update or a FAILED install (non-zero exit) writes no marker, so there is no false restart alert. The marker name matches contract's reader constant byte-for-byte and is DISTINCT from rabbit-auto-evolve's separate `.rabbit-auto-evolve-restart-needed` loop marker. New e2e test `test/test-rabbit-update-restart-marker.py` (real `rabbit-update.py` against a fake rabbit root with a recorder install.py that mutates — or leaves untouched — those surfaces): r0 marker-name parity with contract's reader; r1–r3 changed hook/`CLAUDE.md`/`settings.json` → marker written; r4 non-sensitive-only change → no marker; r5 failed install → no marker. Inv 35(b) extended to document the restart-marker write; `run.py` wired. `scripts/rabbit-update.py` is in `FEATURE_INCLUDES["rabbit-cage"]` — its deployed `.claude/` copy drifts until the dispatcher republishes (Inv 50). Feature/spec/contract versions bumped 5.85.1 -> 5.86.0 in lockstep (Inv 11).

- **v5.85.1 (fix #1092: scrub bare `#NNNN` issue tags from Inv 53 prose):** the Strategy D rewrite shipped in v5.85.0 (#1086) left two bare issue-number tokens in `docs/spec.md`'s Inv 53 body — `(#1052/#1060)` after "This SUPERSEDES Strategy A" and `, #1052` inside the migrate-don't-append clause — tripping the cross-feature contract gate `test-spec-bodies-no-historical-tags.py`, which forbids `#NNNN` tags in feature spec/contract bodies (they belong only in CHANGELOG tombstones and commit messages). DOCS-only scrub: removed just the `#NNNN` tokens, rewording to "This SUPERSEDES the earlier Strategy A, which tracked only `.rabbit/rabbit-project/` ..." and "the SUPERSEDED A-shape tool-tree ignore (`.rabbit/.claude` / `.rabbit/.claude/`) is REMOVED too". Inv 53's meaning, the `write_host_gitignore` behavior, and Strategy D semantics are unchanged; no `install.py` / deployed-surface change. Same recurring class as #1076 (rabbit-spec). Feature/spec/contract versions bumped 5.85.0 -> 5.85.1 in lockstep (Inv 11).

- **v5.85.0 (enh #1086, Strategy D per #1085: vendored host `.gitignore` tracks the WHOLE `.rabbit/`, ignores only ephemerals):** Strategy D (full-vendor) supersedes Strategy A (#1052). Under D a vendored install tracks BOTH the tool tree (`.rabbit/.claude/`) AND the work (`.rabbit/rabbit-project/`) in the host repo, ignoring ONLY the ephemerals (`.rabbit/.runtime/`, plus whatever the inner `.rabbit/.gitignore` covers). Tracking tool+work makes vendored == standalone for VCS: a worktree of the host repo is self-contained, so the proven standalone machinery works unchanged. `install.py`'s `write_host_gitignore(dst_root)` is reworked: the D-shape token set is now the single `.rabbit/.runtime/` (was the A pair `.rabbit/.claude/` + `.rabbit/.runtime/`), and the migration set additionally REMOVES the superseded A-shape `.rabbit/.claude`/`.rabbit/.claude/` tool-tree ignore (alongside the legacy blanket `.rabbit/`/`.rabbit`) so an existing A-shape OR blanket host gitignore is migrated to D rather than shadowed. Idempotent on fresh install and `--update`; standalone installs untouched; unrelated user entries preserved. Inv 53 rewritten from Strategy A to Strategy D (numbering unchanged). The #1052 e2e test `test-install-host-gitignore-strategy-a.py` is replaced by `test-install-host-gitignore-strategy-d.py` (real helper + real git repo via `git check-ignore`): (a) fresh host yields the D-shape — `.rabbit/.claude/...` tracked, `.rabbit/rabbit-project/...` tracked, `.rabbit/.runtime/...` ignored; (b) an A-shape gitignore is migrated to D; (c) a blanket `.rabbit/` ignore is migrated to D; (d) idempotent on re-run, standalone untouched. `run.py` re-wired to the new test. `install.py` is a `publish_file` destination — its deployed repo-root copy drifts until the dispatcher republishes. Feature/spec/contract versions bumped 5.84.0 -> 5.85.0 in lockstep (Inv 11).

- **v5.84.0 (fix #1052, Strategy A per #1060: vendored host `.gitignore` tracks `.rabbit/rabbit-project/`):** the documented vendored-install pattern gitignored the ENTIRE host `.rabbit/` dir (host commit f64fbe2). Because vendored features live under `.rabbit/rabbit-project/features/<name>/`, that blanket ignore made every TDD-cycle git commit silently no-op — no `impl_commit`, no PR; the atomic commit-the-tests-then-impl model was defeated and Step 7 had nothing to PR. The maintainer adopted Strategy A (#1060): track the project work, ignore only the tool tree and ephemerals. New Inv 53: `install.py` gains the exported helper `write_host_gitignore(dst_root)`, called from `main()` after `write_rabbit_gitignore`. In vendored mode (`dst_root.name == ".rabbit"`) it manages the HOST repo `.gitignore` at `dst_root.parent/.gitignore` to ignore EXACTLY `.rabbit/.claude/` and `.rabbit/.runtime/` while leaving `.rabbit/rabbit-project/` tracked; it MIGRATES a pre-existing blanket `.rabbit/`/`.rabbit` line (removes it, not merely shadows it), preserves unrelated user entries, and is idempotent on re-run / `--update`. A standalone install (dst_root is the repo root, not a `.rabbit` subdir) leaves the host `.gitignore` untouched. New e2e test `test-install-host-gitignore-strategy-a.py` (real helper + real git repo via `git check-ignore`). Feature/spec/contract versions bumped 5.83.0 -> 5.84.0 in lockstep (Inv 11).
