...
Inv 54f: the reload-tier advisory is cleared by the `/rabbit-refresh` command's
companion script `scripts/rabbit-refresh-rebaseline.py`, NOT here. A PRIOR fix
(#1173) re-baselined the SKILL.md tier when this dispatcher observed a
`/reload-skills` SUBMITTED prompt — but `/reload-skills` is a Claude Code
CLIENT-LOCAL built-in that reloads SKILL.md definitions in-process and NEVER
fires the UserPromptSubmit hook, so that branch was DEAD (#1193). The re-baseline
now lives on the `/rabbit-refresh` command surface, which IS a real submitted
command whose body runs deterministic `!` bash.
"""
